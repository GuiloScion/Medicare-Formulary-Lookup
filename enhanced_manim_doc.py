# main.py
"""
Enhanced Manim documentary with on-screen subtitles for the full 30-minute script.
Designed for Manim Community v0.16+ / v0.17+ and Google Colab.

Improvements:
- Smoother subtitle transitions with overlapping fades
- Enhanced hyphae with curved branches and natural randomness
- Glow effects for visual depth
- Better particle flow with staggered timing
- Camera movements for cinematic feel
...
def display_subtitles(scene: Scene, lines: list[str], total_time: float):
    """Display subtitle lines evenly over total_time with smooth overlapping transitions."""
    if not lines:
        return
    ...
    for i, txt in enumerate(lines):
        ...
        if prev_sub is not None:
            # Overlapping fade transition for smoother flow
            scene.play(
                FadeOut(prev_sub, shift=DOWN * 0.2),
        ...
        prev_sub = sub
        scene.wait(max(0, per - 0.8))
    
    # Final fadeout
    if prev_sub is not None:
        scene.play(FadeOut(prev_sub, shift=DOWN * 0.2), run_time=0.6)

def add_glow(mobject, color=YELLOW, intensity=0.3, width_multiplier=3):
    """Add a glow effect to any mobject"""
    glow = mobject.copy()
    glow.set_stroke(color, width=mobject.get_stroke_width() * width_multiplier, opacity=intensity)
    glow.set_z_index(mobject.z_index - 1)
    return VGroup(glow, mobject)

def transition_wipe(scene, direction=RIGHT, run_time=0.8):
    """Wipe transition between scenes"""
    wipe = Rectangle(width=15, height=9, fill_opacity=1, color=BLACK)
    wipe.to_edge(LEFT if direction == RIGHT else RIGHT, buff=0)
    scene.play(wipe.animate.shift(direction * 15), run_time=run_time, rate_func=linear)
    scene.remove(wipe)

# Simple drawing primitives used across scenes
def make_tree(size=1.0, trunk_color="#6b4f25", foliage_color=None):
    if foliage_color is None:
        foliage_color = PALETTE["plant"]
    trunk = Rectangle(width=0.12 * size, height=0.7 * size, fill_color=trunk_color, fill_opacity=1, stroke_width=0)
...
class Hyphae(VMobject):
    """Enhanced fractal-like hyphae visual with curved branches."""
    def __init__(self, start=ORIGIN, depth=5, length=1.0, angle_spread=pi/2.2, 
                 color=None, stroke_width=2.0, branching_prob=0.7, **kwargs):
        if color is None:
    ...
    def _build(self):
        ...
        for d in range(self.depth):
            ...
            for p, ang in segments:
                # More natural branching with weighted randomness
                if random.random() < self.branching_prob:
                    n_children = random.choices([1, 2, 3], weights=[0.3, 0.6, 0.1])[0]
                else:
                    n_children = 1
                    
                for i in range(n_children):
                    a = ang + random.uniform(-self.angle_spread/2, self.angle_spread/2) * (1.0 / (d + 1))
                    seg_len = self.length * (0.85 ** d) * random.uniform(0.75, 1.1)
                    q = p + seg_len * np.array([np.cos(a), np.sin(a), 0])
                    
                    # Curved lines instead of straight for organic feel
                    control1 = p + (q - p) * 0.33 + np.random.uniform(-0.08, 0.08, 3) * [1, 1, 0]
                    control2 = p + (q - p) * 0.67 + np.random.uniform(-0.08, 0.08, 3) * [1, 1, 0]
                    curve = CubicBezier(p, control1, control2, q)
                    curve.set_stroke(self.color, width=self.stroke_width * (1.0 / (d + 0.9)))
                    self.append_vectorized_mobject(curve)
                    new_segments.append((q, a))
            segments = new_segments

class NutrientParticles(VGroup):
    def __init__(self, path: VMobject, count=10, color=None, radius=0.04, stagger=True):
        if color is None:
        ...
        for i in range(count):
            ...
            alpha = i / count if not stagger else 0
    ...
    def animate_flow(self, run_time=6, rate_func=linear):
        if self.stagger:
            # Staggered start for continuous flow
            return [
                MoveAlongPath(d, self.path, run_time=run_time, rate_func=rate_func)
                for d in self
            ]
...
class IntroScene(Scene):
    def construct(self):
        ...
        subtitle = Text("Mycorrhizal Symbiosis and the Secret Life of Trees", font_size=34, color=PALETTE["text_secondary"])
        header = VGroup(title, subtitle).arrange(DOWN, buff=0.3).to_edge(UP, buff=0.8)

        # forest skyline with depth (background and foreground)
        forest_bg = VGroup()
        forest_fg = VGroup()
        xs_bg = np.linspace(-7, 7, 5)
        xs_fg = np.linspace(-6.5, 6.5, 8)
        
        for x in xs_bg:
            t = make_tree(size=random.uniform(0.6, 0.9))
            t.shift(RIGHT * x + DOWN * 0.8).set_opacity(0.4)
            forest_bg.add(t)
        ...
        hyphae = Hyphae(start=DOWN * 2.1, depth=6, length=2.2, angle_spread=pi/2, 
                       color=PALETTE["fungal_bright"], stroke_width=2.0, branching_prob=0.75)
        hyphae.shift(DOWN * 0.9)
        
        # Add glow to hyphae
        glowing_hyphae = add_glow(hyphae, color=PALETTE["fungal"], intensity=0.35)
        ...
        particles = NutrientParticles(path, count=12, color=PALETTE["nutrient"], stagger=True)

        self.play(FadeIn(header, shift=UP*0.4), run_time=1.2)
        self.play(LaggedStart(*[GrowFromCenter(t) for t in forest_bg], lag_ratio=0.08), run_time=1.8)
        self.play(LaggedStart(*[GrowFromCenter(t) for t in forest_fg], lag_ratio=0.07), run_time=2.4)
        
        # Parallax effect
        self.play(
            forest_bg.animate.shift(LEFT * 0.3),
            forest_fg.animate.shift(RIGHT * 0.2),
            run_time=2
        )
        
        self.play(Create(glowing_hyphae, run_time=3.5))
        self.play(*particles.animate_flow(run_time=8), run_time=0.3)
...
class HistoryScene(Scene):
    def construct(self):
        left_label = Text("Early plants", font_size=34, color=PALETTE["text_primary"]).to_edge(LEFT).shift(UP*1.0)
        right_label = Text("Fungal hyphae", font_size=34, color=PALETTE["text_primary"]).to_edge(RIGHT).shift(UP*1.0)
        
        early_plants = VGroup()
        for i in range(3):
            stem = Rectangle(width=0.08, height=0.9, fill_color="#9bbf6a", fill_opacity=1).move_to(LEFT*2.4 + RIGHT*(i*0.3) + DOWN*0.6)
            leaf = Ellipse(width=0.7, height=0.3, fill_color=PALETTE["plant_light"], fill_opacity=1).next_to(stem, UP, buff=0.05)
            early_plants.add(stem, leaf)
            
        fungal_cluster = Hyphae(start=RIGHT*1.5 + DOWN*0.6, depth=5, length=1.1, 
                               angle_spread=pi/1.8, color="#48b2b2", stroke_width=2.2, branching_prob=0.7)
        
        arrow1 = Arrow(start=LEFT*1.2, end=RIGHT*0.6, buff=0.5, stroke_width=3.0).set_color(PALETTE["nutrient"])
        ...
        self.play(Create(fungal_cluster, run_time=2.8))
        
        # move arrows into view
        arrow1.move_to(DOWN * 0.25)
        arrow2.move_to(DOWN * 0.75)
        self.play(GrowArrow(arrow1), GrowArrow(arrow2), run_time=1.2)
        ...
        for i in range(len(dots)):
            for j in range(i + 1, len(dots)):
                if random.random() < 0.6:
                    edge = Line(dots[i].get_center(), dots[j].get_center()).set_stroke(PALETTE["network"], width=2.0, opacity=0.95)
                    edges.add(edge)
                    graph.add(edge)
                    
        self.play(LaggedStartMap(FadeIn, graph, shift=UP, lag_ratio=0.08), run_time=2.2)
        
        # Camera zoom for cinematic effect
        self.play(self.camera.frame.animate.scale(0.8).move_to(dots[2]), run_time=2)
        self.wait(1)
        self.play(self.camera.frame.animate.scale(1/0.8).move_to(ORIGIN), run_time=2)
        
        display_subtitles(self, SUBS["NetworkOverviewScene"], SCENE_DURATIONS["NetworkOverviewScene"])
        
        # animate pulses on a subset of edges
        pulses = []
        for edge in np.random.choice([e for e in edges], size=min(7, len(edges)), replace=False):
            dot = Dot(radius=0.06, color=PALETTE["nutrient"]).move_to(edge.get_start())
            self.add(dot)
...
class MiningExchangeScene(Scene):
    def construct(self):
        ...
        hypha = Hyphae(start=DOWN*1.5 + RIGHT*1.5, depth=6, length=1.3, 
                      angle_spread=pi/1.9, color="#3dbdbd", stroke_width=2.0, branching_prob=0.75)
        hypha.shift(RIGHT*0.8)
        
        # Scale indicator
        scale_text = Text("1000× magnified", font_size=20, color=GRAY).to_corner(DR)
        
        self.play(FadeIn(ground), GrowFromCenter(tree))
        self.add(scale_text)
        self.play(Create(hypha, run_time=2.8))
        
        p_icon = MathTex("P", font_size=64, color=PALETTE["nutrient"]).move_to(hypha.get_center() + LEFT*0.8 + DOWN*0.4)
        n_icon = MathTex("N", font_size=64, color=PALETTE["nutrient"]).move_to(hypha.get_center() + RIGHT*0.9 + DOWN*0.4)
        sugar = MathTex("C_{6}H_{12}O_{6}", font_size=28, color=PALETTE["carbon"]).next_to(tree, DOWN, buff=0.1)
        ...
        p_dot = Dot(radius=0.06, color=PALETTE["nutrient"]).move_to(p_icon.get_center())
        n_dot = Dot(radius=0.06, color=PALETTE["nutrient"]).move_to(n_icon.get_center())
        sugar_dot = Dot(radius=0.06, color=PALETTE["carbon"]).move_to(sugar.get_center())
...
class CommunicationScene(Scene):
    def construct(self):
        ...
        root1 = Line(t1.get_bottom(), t2.get_bottom()).set_stroke(PALETTE["network"], width=3.0)
        root2 = Line(t2.get_bottom(), t3.get_bottom()).set_stroke(PALETTE["network"], width=3.0)
        self.play(Create(root1), Create(root2))
        
        # insect attack with expanding wave
        self.play(t1.animate.set_color(PALETTE["alert"]).scale(1.04), run_time=0.6)
        
        # Expanding wave effect instead of single dot
        for i in range(3):
            wave = Circle(radius=0.2, stroke_color=PALETTE["alert"], stroke_width=3)
            wave.move_to(t1.get_bottom())
            self.play(
                wave.animate.scale(6).set_stroke(opacity=0),
                run_time=1.0
            )
        
        alarm = Dot(radius=0.07, color=PALETTE["alert"]).move_to(t1.get_bottom())
        self.add(alarm)
        path = Line(alarm.get_center(), t3.get_bottom())
        self.play(MoveAlongPath(alarm, path, run_time=4.2, rate_func=linear))
        
        shield = Tex("✦", font_size=48, color=PALETTE["defense"]).next_to(t3, UP, buff=0.1)
...
class ArchitectureScene(Scene):
    def construct(self):
        ...
        root_label = Text("Root Cross-Section", font_size=30, color=PALETTE["text_primary"]).to_edge(UP)
...
class CompetitionScene(Scene):
    def construct(self):
        network = Hyphae(start=DOWN*1.0, depth=5, length=1.4, angle_spread=pi/1.6, 
                        color="#4db6ac", stroke_width=2.0, branching_prob=0.7)
        self.play(Create(network, run_time=2.2))
        
        host = make_tree(size=1.3).move_to(LEFT*2 + DOWN*0.2)
        parasite = make_tree(size=0.9).move_to(RIGHT*2 + DOWN*0.2).set_color(PALETTE["alert"])
        ...
        host_dot = Dot(radius=0.06, color=PALETTE["nutrient"]).move_to(host.get_bottom())
...
class EcosystemsScene(Scene):
    def construct(self):
        left_title = Text("Temperate forests", font_size=34, color=PALETTE["text_primary"]).to_edge(LEFT).shift(UP*1.2)
        right_title = Text("Tropical soils", font_size=34, color=PALETTE["text_primary"]).to_edge(RIGHT).shift(UP*1.2)
        ...
        left_h = Hyphae(start=left_circle.get_center() + DOWN*0.7, depth=6, length=1.05, 
                       angle_spread=pi/1.9, color="#2f8d8d", branching_prob=0.75)
        right_h = Hyphae(start=right_circle.get_center() + DOWN*0.7, depth=4, length=0.75, 
                        angle_spread=pi/1.2, color="#f2b46e", branching_prob=0.65)
...
class ResearchMethodsScene(Scene):
    def construct(self):
        ...
        seq_label = Text("DNA Sequencing", font_size=30, color=PALETTE["text_primary"]).to_edge(LEFT).shift(UP*0.6)
        iso_label = Text("Isotopic Tracing", font_size=30, color=PALETTE["text_primary"]).to_edge(RIGHT).shift(UP*0.6)
        ...
        net = Line(t1.get_center(), t2.get_center()).set_stroke(PALETTE["network"], width=3.0)
        
        self.play(FadeIn(t1), FadeIn(t2), Create(net))
        
        # Enhanced tracer with glow
        tracer = Dot(radius=0.08, color="#b388eb").move_to(t1.get_center())
        tracer_glow = tracer.copy().set_color("#b388eb").scale(2).set_opacity(0.3)
        tracer_group = VGroup(tracer_glow, tracer)
        
        self.add(tracer_group)
        self.play(
            tracer_group.animate.move_to(t2.get_center()),
            run_time=5.6,
            rate_func=smooth
        )
...
class ThreatsRestorationScene(Scene):
    def construct(self):
        ...
        left_label = Text("Threats", font_size=36, color=PALETTE["alert"], weight=BOLD).next_to(left_back, UP, buff=0.3)
        right_label = Text("Restoration", font_size=36, color="#8ee4af", weight=BOLD).next_to(right_back, UP, buff=0.3)
        ...
        self.play(
            Create(left_back), Create(right_back),
        ...
        self.play(Create(broken, run_time=1.5))
        
        # Restoration: inoculation point
        inoc = Dot(radius=0.09, color=PALETTE["nutrient"]).move_to(right_back.get_center() + DOWN*1.2)
        inoc_glow = Circle(radius=0.15, color=PALETTE["nutrient"], fill_opacity=0.3, stroke_width=0).move_to(inoc.get_center())
        
        self.play(FadeIn(VGroup(inoc_glow, inoc)))
        
        # Growing new hyphae
        new_hypha = Hyphae(start=right_back.get_center() + DOWN*1.2, depth=6, length=1.1, 
                          angle_spread=pi/1.8, color="#4db6ac", stroke_width=2.0, branching_prob=0.8)
        glowing_new_hypha = add_glow(new_hypha, color="#4db6ac", intensity=0.4)
        
        self.play(Create(glowing_new_hypha, run_time=3.5))
...
class ConclusionScene(Scene):
    def construct(self):
        title = Text("Everything is connected.", font_size=72, color=PALETTE["text_primary"], weight=BOLD)
        subtitle = Text("Heal the networks. Heal the planet.", font_size=34, color=PALETTE["text_secondary"])
        group = VGroup(title, subtitle).arrange(DOWN, buff=0.5)
        
        # Subtle background network
        bg_network = VGroup()
        for _ in range(15):
            x, y = random.uniform(-6, 6), random.uniform(-3, 3)
            dot = Dot(point=[x, y, 0], radius=0.03, color=PALETTE["fungal"], fill_opacity=0.2)
            bg_network.add(dot)
        
        # Connect some dots
        for i in range(20):
            if len(bg_network) > 1:
                d1, d2 = random.sample(list(bg_network), 2)
                if np.linalg.norm(d1.get_center() - d2.get_center()) < 3:
                    line = Line(d1.get_center(), d2.get_center())
                    line.set_stroke(PALETTE["fungal"], width=0.5, opacity=0.15)
                    bg_network.add(line)
        
        self.add(bg_network)
        self.play(FadeIn(group, shift=UP*0.6), run_time=1.4)
        
        # Pulse the background network
        for _ in range(2):
            self.play(
                bg_network.animate.set_opacity(0.4),
                run_time=1.5
            )
            self.play(
                bg_network.animate.set_opacity(0.15),
                run_time=1.5
            )
        ...
        # Final fade to black
        self.play(FadeOut(VGroup(group, bg_network)), run_time=3.0)
        self.wait(1.0)


# ---------------- FULL DOCUMENTARY SEQUENCE ----------------
class FullDocumentary(Scene):
    """Renders all scenes in sequence - use this for the complete 30-minute documentary"""
    def construct(self):
        scenes = [
            IntroScene,
            HistoryScene,
        ...
        for i, scene_class in enumerate(scenes):
            # Transition wipe between scenes (except first)
            if i > 0:
                transition_wipe(self, direction=RIGHT, run_time=0.6)
            
            # Create temporary scene instance to borrow its construct method
            temp_scene = scene_class()
            # Copy the construct logic by calling it with our context
            temp_scene.renderer = self.renderer
            temp_scene.camera = self.camera
...
# Cell 1: Install Manim
!sudo apt update
!sudo apt install libcairo2-dev ffmpeg texlive texlive-latex-extra texlive-fonts-extra texlive-latex-recommended texlive-science tipa libpango1.0-dev
!pip install manim

# Cell 2: Save this script
%%writefile main.py
# [paste entire script here]

# Cell 3: Render individual scenes (for testing)
!manim -pql main.py IntroScene

# Cell 4: Render all scenes separately
!manim -pql main.py IntroScene
...
# Cell 5: Render full documentary (WARNING: Takes ~2-3 hours)
!manim -pqh main.py FullDocumentary

Quality options:
-pql  = Low quality (480p, 15fps) - fastest, for testing
-pqm  = Medium quality (720p, 30fps) - balanced
-pqh  = High quality (1080p, 30fps) - production
-pqk  = 4K quality (2160p, 60fps) - highest quality, very slow

Performance tips:
- Use -pql for testing individual scenes
- Render scenes separately, then combine with video editor
- For full 30-min render, use Google Colab Pro for longer runtime
"""